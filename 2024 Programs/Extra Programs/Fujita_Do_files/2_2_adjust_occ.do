/* This code creates occupation codes that are consistent over time. The original code is 
is by Meyer and Osborne (2005). We use fairly coase occupation breakdown, which is saved in 
the varaible mjrocc. 
*/
clear
set more off 
...
if `yearmth' >= 196801 & `yearmth' < 197101{
    scalar Cencode60=1
}
if `yearmth' >= 197101 & `yearmth' < 198301{
    scalar Cencode60=0
	scalar Cencode70=1
}
if `yearmth' >= 198301{
...
if `yearmth' >= 199101{
	scalar Cencode60=0
	scalar Cencode70=0
    scalar Cencode80=2  /* there were a few redefinitions */
}
if `yearmth' >= 199501{
	scalar Cencode60=0
	scalar Cencode70=0
...
if `yearmth' <201101 & `yearmth' >= 200301{
	scalar Cencode60=0
	scalar Cencode70=0
    scalar Cencode80=0 
	scalar Cencode00=1
...
recast double ocsrc
*This is important becaue if ocsrc is float, if condition with decimal, e.g., 5.1 does not work
...
/*
IMPORTANT NOTE: OCCUPATION CODES FOR 2020 ON ARE CHANGED TO CENSUS 18 CODES. BUT I AM ACCOMODATING
THESE CHANGES WITHIN Cencode10 (USING THE CROSSWALK BETWEEN CENSUS 10 AND CENSUS 18), SINCE THEY ARE SIMILAR, AND CREATING THE SEPARATE MACRO VARIABLE REQUIRES ME TO MAKE CHANGES TO THIS STATA CODE FOR ALL OCCUPATION CATEGORIES.  
*/
 
/* Legislators */
replace ocdest=3 if
  (Cencode80 & ocsrc==3) |
  ((Cencode00|Cencode10) & ocsrc==3);

/* Chief executives and public administrators */
replace ocdest=4 if
  (Cencode60 & ocsrc==270) |
  (Cencode80 & ocsrc==4) |
  ((Cencode00|Cencode10) & ocsrc==1);
...
replace ocdest=7 if
  ...
  (Cencode80 & ocsrc==7) |
  ((Cencode00|Cencode10) & ocsrc==12);

/* Human resources and labor relations managers */
replace ocdest=8 if
  (Cencode80 & ocsrc==8) |
  (Cencode00 & ocsrc==13)|
  (Cencode10 & (ocsrc>=13.5 & ocsrc<=13.7));  /*compensation and benefits managers, human resources managers, training and development managers */
...
replace ocdest=13 if
  ...
  ((Cencode00|Cencode10) & (ocsrc==4|ocsrc==5|ocsrc==6|ocsrc==5.1|ocsrc==5.2)) | 
  (Cencode00 & ocsrc==282) |
  (Cencode10 & ocsrc==282.5);
...
replace ocdest=16 if
  (Cencode60 & ocsrc==280)   |
  (Cencode70 & ocsrc==224)   |
  (Cencode80==1 & ocsrc==17) |
  (Cencode80>1 & ocsrc==16) |
  ((Cencode00|Cencode10) & ocsrc==40) ;

/* Managers of food-serving and lodging establishments */
replace ocdest=17 if
  (Cencode70 & ocsrc==230) |
  (Cencode80>1 & ocsrc==17) |
  ((Cencode00|Cencode10) & ocsrc==31) | 
  ((Cencode00|Cencode10) & ocsrc==34);
...
replace ocdest=18 if
  ...
  (Cencode70 & ocsrc==216) |
  (Cencode80==1 & ocsrc==16) |
  (Cencode80>1 & ocsrc==18) |
  ((Cencode00|Cencode10) & ocsrc==41);
...
replace ocdest=19 if
  ...
  (Cencode70    & ocsrc==211) |
  (Cencode80==1 & ocsrc==18) |
  (Cencode80>=2 & ocsrc==19) |
  (Cencode00 & ocsrc==32) |
  (Cencode10 & (ocsrc==32.5|ocsrc==446.5));

/* Managers of service organizations, n.e.c. */
replace ocdest=21 if
  (Cencode80>1 & ocsrc==21) |
  ((Cencode00|Cencode10) & (ocsrc==33 | ocsrc==33.5)) | 
  ((Cencode00|Cencode10) & ocsrc==36) |    /* natural science managers */
  ((Cencode00|Cencode10) & ocsrc==42) | 
  (Cencode00 & ocsrc==72) |
  (Cencode10 & ocsrc==72.5);    /* meeting & convention planners */
...
replace ocdest=23 if
  ...
  (Cencode70 & ocsrc==1)  |
  (Cencode80 & ocsrc==23) |
  ((Cencode00|Cencode10) & ocsrc==80) |
  ((Cencode00|Cencode10) & ocsrc==93); /* tax examiners, collectors, revenue agents */

/* Insurance underwriters */
replace ocdest=24 if
  (Cencode80 & ocsrc==24) |
  ((Cencode00|Cencode10) & ocsrc==86);
...
replace ocdest=36 if
  ...
  (Cencode80 & ocsrc==36) |
  ((Cencode00|Cencode10) & ((ocsrc==56) | (ocsrc==90))) |
  (Cencode10 & ocsrc==56.5);

/* Management related jobs (e.g. support) */
replace ocdest=37 if
  (Cencode80 & ocsrc==37) |
  (Cencode00 & ocsrc==73) | /* other business operations specialists */
  (Cencode10 & (ocsrc==42.5|ocsrc==74|ocsrc==75|ocsrc==70.5));  /*emergency management directors and business operations specialists, all others; 75 and 70.5 are new in 2020 */ 
...
replace ocdest=43 if
  ...
  (Cencode70 & ocsrc==2)   |
  (Cencode80 & ocsrc==43)  |
  ((Cencode00|Cencode10) & (ocsrc==130|ocsrc==130.5|ocsrc==130.6)); /* Architects, exc.naval, 130.6 and 130.5 are new in 2020 */
...
replace ocdest=45 if
  ...
  (Cencode70 & ocsrc==15) |
  (Cencode80 & ocsrc==45) |
  ((Cencode00|Cencode10) & ocsrc==145);   /* materials engineers */
...
replace ocdest=47 if
  ...
  (Cencode70 & ocsrc==21)  |
  (Cencode80 & ocsrc==47)  |
  ((Cencode00|Cencode10) & ocsrc==152);   /* Petroleum, mining, mining safety, and geological engrs */
...
replace ocdest=55 if
  ...
  (Cencode70 & ocsrc==12) |
  (Cencode80 & ocsrc==55) |
  ((Cencode00|Cencode10) & ocsrc ==140) | /* computer hardware engrs */
  ((Cencode00|Cencode10) & ocsrc ==141);  /* elect &electronic engrs */
...
replace ocdest=56 if
  ...
  (Cencode70 & ocsrc==13) |
  (Cencode80 & ocsrc==56) |
  ((Cencode00|Cencode10) & ocsrc==143);    /* IEs, including health and safety */
...
replace ocdest=59 if
  ...
  ((Cencode00|Cencode10) & ((ocsrc==133)|(ocsrc==134)|(ocsrc==142)|(ocsrc==144)|(ocsrc==150)|(ocsrc==151)|(ocsrc==152)|(ocsrc==153)));
...
replace ocdest=64 if
  ...
  (Cencode00 & ocsrc==100) |
  (Cencode10 & (ocsrc==105|ocsrc==106|ocsrc==106.5|(ocsrc>=100.5&ocsrc<=100.7)|ocsrc==103|ocsrc==103.1|ocsrc==103.2|ocsrc==110.5|(ocsrc>=110.55 & ocsrc<=110.7)|ocsrc==110.8));  
...
replace ocdest=65 if
  ...
  ((Cencode00|Cencode10) & ocsrc==70) |    /* Logisticians */
  ((Cencode00|Cencode10) & ocsrc==122);   /* Operations research analysts*/

/* Actuaries */
replace ocdest=66 if
  (Cencode70 & ocsrc==34) |
  (Cencode80 & ocsrc==66)  |
  ((Cencode00|Cencode10) & ocsrc==120);
...
replace ocdest=67 if
  ...
  (Cencode70 & ocsrc==36) |
  (Cencode80 & ocsrc==67) |
  ((Cencode00|Cencode10) & ocsrc==123);

/* Mathematical scientists and mathematicians */
replace ocdest=68 if
  (Cencode60 & ocsrc==135) | (Cencode70 & ocsrc==35) |
  (Cencode80 & ocsrc==68)  |
  ((Cencode00|Cencode10) & ocsrc==121) |   /* mathematicians */
  ((Cencode00|Cencode10) & ocsrc==124);    /* misc mathematical scientists */
...
replace ocdest=73 if 
  ...
  (Cencode70 & ocsrc==45) |
  (Cencode80 & ocsrc==73) |
  ((Cencode00|Cencode10) & ocsrc==172);

/* Atmospheric and space scientists */
replace ocdest=74 if 
  (Cencode70 & ocsrc==043) |
  (Cencode80 & ocsrc==74 ) |
  ((Cencode00|Cencode10) & ocsrc==171);
...
replace ocdest=75 if
  ...
  (Cencode70 & ocsrc==51)  |
  (Cencode80 & ocsrc==75)  |
  ((Cencode00|Cencode10) & ocsrc==152) |
  ((Cencode00|Cencode10) & (ocsrc==174| ocsrc==174.5| ocsrc==175));    /* enviro scientists and geoscientists */
...
replace ocdest=79 if
  ...
  (Cencode70 & ocsrc==025) |
  (Cencode80 & ocsrc==79)  |
  ((Cencode00|Cencode10) & ocsrc==164);

/* Medical scientists */
replace ocdest=83 if
  (Cencode80 & ocsrc==83) | 
  ((Cencode00|Cencode10) & ocsrc==165);
...
replace ocdest=84 if
  ...
  (Cencode70 & ocsrc==065) |
  (Cencode80 & ocsrc==84)  |
  ((Cencode00|Cencode10) & (ocsrc==306| ocsrc==306.5|ocsrc==307|ocsrc==309|ocsrc==310));  /* Physicians and surgeons */
...
replace ocdest=87 if
  ...
  (Cencode70 & ocsrc==063) |
  (Cencode80 & ocsrc==87)  |
  ((Cencode00|Cencode10) & ocsrc==304);

/* Podiatrists */
replace ocdest=88 if
  (Cencode70 & ocsrc==71) |
  (Cencode80 & ocsrc==88) |
  ((Cencode00|Cencode10) & ocsrc==312);
...
replace ocdest=89 if
  ...
  ((Cencode00|Cencode10) & ocsrc==300) |  /* chiropractors */
  ((Cencode00|Cencode10) & (ocsrc==326|ocsrc==327|ocsrc==326.1));   /* other health diagnosis & treatment */
...
replace ocdest=95 if
  ...
  (Cencode80 & ocsrc==95)  |
  (Cencode00 & ocsrc==313) |
  (Cencode10 & (ocsrc>=325.5 & ocsrc<=325.8));
...
replace ocdest=97 if
  ...
  (Cencode70 & ocsrc==74) |
  (Cencode80 & ocsrc==97) |
  ((Cencode00|Cencode10) & ocsrc==303);

/* Respiratory therapists */
replace ocdest=98 if
  (Cencode80 & ocsrc==98) |
  ((Cencode00|Cencode10) & ocsrc==322);

/* Occupational therapists */
replace ocdest=99 if
  (Cencode80 & ocsrc== 99) |
  ((Cencode00|Cencode10) & ocsrc==315) |
  ((Cencode00|Cencode10) & ocsrc==361);    /* occ therapists assts and aides */

/* Physical therapists */
replace ocdest=103 if
  (Cencode80 & ocsrc==103) |
  ((Cencode00|Cencode10) & ocsrc==316) |
  ((Cencode00|Cencode10) & ocsrc==362);    /* phys therapists assts and aides */

/* Speech therapists */
replace ocdest=104 if
  (Cencode80 & ocsrc==104) |   /* speech therapists */
  ((Cencode00|Cencode10) & ocsrc==314) |   /* audiologists */
  ((Cencode00|Cencode10) & ocsrc==323);    /* speech language pathologists */
...
replace ocdest=105 if
  ...
  ((Cencode00|Cencode10) & ocsrc==320) |   /* radiation therapists */
  ((Cencode00|Cencode10) & ocsrc==321) |   /* recreational therapists */
  ((Cencode00|Cencode10) & (ocsrc>=323.5&ocsrc<=324.5));    /* therapists, all other */

/* Physicians' assistants */
replace ocdest=106 if
  (Cencode80 & ocsrc==106) |
  ((Cencode00|Cencode10) & ocsrc==311);
...
replace ocdest=154 if
  ...
  (Cencode80 & (
   ...
   (ocsrc==146) | (ocsrc==148) |
   (ocsrc==153) | (ocsrc==154))) |
  ((Cencode00|Cencode10) & (ocsrc==220|ocsrc==220.5)); 

/* Kindergarten and earlier school teachers */
replace ocdest=155 if
  (Cencode70 & ocsrc==143) |
  (Cencode80 & ocsrc==155) |
  ((Cencode00|Cencode10) & ocsrc==230);  /* preschool and kindergarten */
...
replace ocdest=157 if
  ...
  (Cencode70 & ocsrc==144) |
  (Cencode80 & ocsrc==157) |
  ((Cencode00|Cencode10) & ocsrc==232);

/* Special education teachers */
replace ocdest=158 if
  (Cencode80 & ocsrc==158) |
  ((Cencode00|Cencode10) & ocsrc==233);
...
replace ocdest=159 if
  ...
  (Cencode70 & ocsrc==145) |
  (Cencode80 & ocsrc==159) |
  ((Cencode00|Cencode10) & (ocsrc==234| ocsrc==235|ocsrc==236)) | 
   ((Cencode00|Cencode10) & ( ocsrc==254 | ocsrc==254.5 | ocsrc==255| ocsrc==243.5 | ocsrc==255.5));

/* Vocational and educational counselors */
replace ocdest=163 if
  (Cencode70 & ocsrc==174) |
  (Cencode80 & ocsrc==163) |
  ((Cencode00|Cencode10) & (ocsrc==200|ocsrc==200.1|ocsrc==200.2|ocsrc==200.3|ocsrc==200.4|ocsrc==200.5|ocsrc==200.6));

/* Librarians */
replace ocdest=164 if 
  (Cencode60 & ocsrc==111)  |
  (Cencode70 & ocsrc==32)  |
  (Cencode80 & ocsrc==164) |
  ((Cencode00|Cencode10) & (ocsrc==243|ocsrc==243.5));

/* Archivists and curators */
replace ocdest=165 if
  (Cencode70 & ocsrc==33)  |
  (Cencode80 & ocsrc==165) |
  ((Cencode00|Cencode10) & ocsrc==240);
...
replace ocdest=166 if
  ...
  (Cencode70 & ocsrc==91)  |
  (Cencode80 & ocsrc==166) |
  ((Cencode00|Cencode10) & ocsrc==180) |   /* economists */
  (Cencode00 & ocsrc==181) |
  (Cencode10 & (ocsrc==73.5|ocsrc==181.5));    /* market and survey researcers */
...
replace ocdest=167 if
  ...
  (Cencode70 & ocsrc==93) |
  (Cencode80 & ocsrc==167) |
  ((Cencode00|Cencode10) & (ocsrc==182 | ocsrc==182.1 | ocsrc==182.2 |ocsrc==182.5));

/* Sociologists */
replace ocdest=168 if
  (Cencode70 & ocsrc==094) |
  (Cencode80 & ocsrc==168) |
  ((Cencode00|Cencode10) & ocsrc==183);    /* sociologists */
...
replace ocdest=169 if
  ...
  (Cencode80 & ocsrc==169)  |
  ((Cencode00|Cencode10) & ocsrc==186);   /* misc social scientists */

/* Urban and regional planners */
replace ocdest=173 if
  (Cencode70 & ocsrc==095) |
  (Cencode80 & ocsrc==173) |
  ((Cencode00|Cencode10) & ocsrc==184);
...
replace ocdest=174 if
  ...
  (Cencode70 & ocsrc==100) |
  (Cencode80 & ocsrc==174) |
  ((Cencode00|Cencode10) & (ocsrc==201| ocsrc==201.1|ocsrc==201.2|ocsrc==201.3|ocsrc==201.4));
...
replace ocdest=175 if
  ...
  (Cencode70 & ocsrc==101) |
  (Cencode80 & ocsrc==175) |
  ((Cencode00|Cencode10) & (ocsrc==462|ocsrc==462.1|ocsrc==462.2));
...
replace ocdest=176 if
  ...
  (Cencode80 & ocsrc==176) |
  (Cencode80 & ocsrc==177) |
  ((Cencode00|Cencode10) & ocsrc==204) |
  ((Cencode00|Cencode10) & ocsrc==205) |
  ((Cencode00|Cencode10) & ocsrc==206);
...
replace ocdest=178 if
  ...
  (Cencode70 & ocsrc==31)  |
  (Cencode80 & ocsrc==178) |
  ((Cencode00|Cencode10) & ocsrc==210);

/* Judges */
replace ocdest=179 if
  (Cencode70 & ocsrc==30)  |
  (Cencode80 & ocsrc==179) |
  ((Cencode00|Cencode10) & ocsrc==211);  /* Judges, magistrates, and other judicial workers */
...
replace ocdest=183 if
  ...
  (Cencode70 & ocsrc==181) |
  (Cencode80 & ocsrc==183) |
  ((Cencode00|Cencode10) & ocsrc==285);

/* Technical writers */
replace ocdest=184 if
  (Cencode80 & ocsrc==184) |
  ((Cencode00|Cencode10) & ocsrc==284);
...
replace ocdest=185 if
  ...
  (Cencode80 & ocsrc==185) |
  ((Cencode00|Cencode10) & (ocsrc==263| (ocsrc>=263.1 & ocsrc<=263.6) | ocsrc==264));
...
replace ocdest=186 if
  ...
  (Cencode70 & ocsrc==185) |
  (Cencode80 & ocsrc==186) |
  ((Cencode00|Cencode10) & (ocsrc==275 | ocsrc==275.1 |ocsrc==275.2)); /* Musicians, singers,and related workers */
...
replace ocdest=187 if
  ...
  (Cencode70 & ocsrc==175) |
  (Cencode80 & ocsrc==187) |
  ((Cencode00|Cencode10) & ocsrc==270) | /* Actors */
  ((Cencode00|Cencode10) & ocsrc==271);  /* Producers & directors */
...
replace ocdest=193 if
  ...
  (Cencode70 & ocsrc==182)  |
  (Cencode80 & ocsrc==193) |
  ((Cencode00|Cencode10) & ocsrc==274); /* ... and choreographers */
...
replace ocdest=194 if
  ...
  (Cencode80 & ocsrc==194) |
  ((Cencode00|Cencode10) & (ocsrc==276|ocsrc==275.5|ocsrc==277)) |
  ((Cencode00|Cencode10) & (ocsrc==286|ocsrc==286.1|ocsrc==286.5));    /* misc media and communications wkrs */
...
replace ocdest=195 if
  ...
  (Cencode70 & ocsrc==184) |
  (Cencode80 & ocsrc==195) |
  ((Cencode00|Cencode10) & ocsrc==281) |  /* News analysts,reporters,correspondents */
  ((Cencode00|Cencode10) & ocsrc==283) |  /* Editors */
  ((Cencode00|Cencode10) & ocsrc==292);   /* TV video, and motion picture camera */
...
replace ocdest=198 if
  ...
  (Cencode80 & ocsrc==198) |
  ((Cencode00|Cencode10) & (ocsrc==280|ocsrc==280.5));
...
replace ocdest=199 if
  ...
  (Cencode70 & ocsrc==180) |
  (Cencode80 & ocsrc==199) |
  ((Cencode00|Cencode10) & (ocsrc==272 |(ocsrc>=272.1 & ocsrc<=272.3))) | /* Athletes, coaches, umpires, and related */
  ((Cencode00|Cencode10) & ocsrc==752);  /* commercial divers */
...
replace ocdest=203 if
  ...
  (Cencode70 & ocsrc== 80) |
  (Cencode80 & ocsrc==203) |
  ((Cencode00|Cencode10) & ocsrc==330);

/* Dental hygenists */
replace ocdest=204 if
  (Cencode70 & ocsrc==081) |
  (Cencode80 & ocsrc==204) |
  ((Cencode00|Cencode10) & ocsrc==331);

/* Health record technologists and technicians */
replace ocdest=205 if
  (Cencode70 & ocsrc==82) |
  (Cencode80 & ocsrc==205) |
  ((Cencode00|Cencode10) & (ocsrc==351|ocsrc==351.5));

/* Radiologic technologists and technicians */
replace ocdest=206 if
  (Cencode70 & ocsrc==083) |
  (Cencode80 & ocsrc==206) |
  ((Cencode00|Cencode10) & (ocsrc==332| ocsrc==332.1|ocsrc==332.2|ocsrc==332.3|ocsrc==332.4 |ocsrc==333));
...
replace ocdest=207 if
  ...
  (Cencode70 & ocsrc==926) |
  (Cencode80 & ocsrc==207) |
  ((Cencode00|Cencode10) & ocsrc==350);

/* Health technologists and technicians, n.e.c. */
replace ocdest=208 if
  (Cencode70 & ocsrc==85) |
  (Cencode80 & ocsrc==208) |
  ((Cencode00|Cencode10) & (ocsrc==340|ocsrc==340.1|ocsrc==340.2)) |   /* emergency medical techs & paramedics */
  (Cencode00 & ocsrc==353) |
  ((Cencode00|Cencode10) & (ocsrc==354 |ocsrc==355 |ocsrc==198)) |
  (Cencode10 & (ocsrc==353.5|ocsrc==354.5));
...
replace ocdest=214 if
  ...
  (Cencode80 & ocsrc==214) |
  (Cencode80 & ocsrc==216) |
  ((Cencode00|Cencode10) & (ocsrc==155 | ocsrc==155.1| ocsrc==155.5)) | /* Engineering technicians, except drafters */
  (Cencode00 & ocsrc==196) |
  (Cencode10 & (ocsrc==195|ocsrc==196.5|ocsrc==197));  /* life, physical, and soc sci techs -- */
                                /* oddly the Census 1990/2000 says most of */
                                /* those go here not to sci techs (225).  197 is new */
...
replace ocdest=217 if
  ...
  (Cencode70 & ocsrc==152) |
  (Cencode80 & ocsrc==217) |
  ((Cencode00|Cencode10) & (ocsrc==154 | ocsrc==154.1 | ocsrc==154.5));   /* Drafters  154.1 and 154.2 are new in 2020*/
...
replace ocdest=218 if
  ...
  (Cencode80 & ocsrc==218) |
  (Cencode80 & ocsrc==867) |
  ((Cencode00|Cencode10) & ocsrc==131) | /* Surv.., cart.., and photogrammetrists */
  ((Cencode00|Cencode10) & ocsrc==156);  /* Surveying & mapping techs */

/* Biological and agricultural technicians */
replace ocdest=223 if
  (Cencode70 & ocsrc==150) | /* ag and bio techs */
  (Cencode80 & ocsrc==223) |
  ((Cencode00|Cencode10) & (ocsrc==190 | ocsrc==191)); /* ag and food science techs */

/* Chemical technicians */
replace ocdest=224 if
  (Cencode70 & ocsrc==151) |
  (Cencode80 & ocsrc==224) |
  ((Cencode00|Cencode10) & ocsrc==192);
...
replace ocdest=225 if
  ...
  ((Cencode00|Cencode10) & (ocsrc==193|ocsrc==193.5)); 
...
replace ocdest=226 if
  ...
  (Cencode80 & ocsrc==226) |
  ((Cencode00|Cencode10) & ocsrc==903);

/* Air traffic controllers */
replace ocdest=227 if
  (Cencode70 & ocsrc==164) |
  (Cencode80 & ocsrc==227) |
  ((Cencode00|Cencode10) & ocsrc==904);
...
replace ocdest=228 if
  ...
  (Cencode80 & ocsrc==228) |
  ((Cencode00|Cencode10) & (ocsrc==290|ocsrc==290.5)) |   /* tho many map to technicians, n.e.c. */
  ((Cencode00|Cencode10) & (ocsrc==296|ocsrc==290.5|ocsrc==297)); /* media and communications workers, all other */

/* Computer software developers */
replace ocdest=229 if
  (Cencode70 & ocsrc==3) |
  (Cencode80 & ocsrc==229) |
  ((Cencode00|Cencode10) & ocsrc==101) | /* computer programmers */
  ((Cencode00|Cencode10) & (ocsrc==102 | ocsrc==102.1 | ocsrc==102.2)); /* computer software engineers 102.1 and 102.2 are new in 2020 */

/* Programmers of numerically controlled machine tools */
replace ocdest=233 if
  (Cencode70 & ocsrc==172) |
  (Cencode80 & ocsrc==233) |
  ((Cencode00|Cencode10) & (ocsrc==790|ocsrc==790.5));

/* Legal assistants, paralegals, legal support, etc */
replace ocdest=234 if
  (Cencode80 & ocsrc==234) |
  (Cencode00 & ((ocsrc==214) | (ocsrc==215))) |
  (Cencode10 & (ocsrc==214.5|ocsrc==210.5|ocsrc==216|ocsrc==217|ocsrc==218|ocsrc==286.2)); 
/* Technicians, n.e.c. */
replace ocdest=235 if
  ...
  (Cencode80 & ocsrc==235) |
  ((Cencode00|Cencode10) & ocsrc==194);  /* nuclear technicians */

/* Supervisors and proprietors of sales jobs */
replace ocdest=243 if
  (Cencode60 & ocsrc==254) |
  (Cencode80 & ocsrc==243) |
  ((Cencode00|Cencode10) & ((ocsrc==470)|(ocsrc==471)));    /* supvs of sales */
...
replace ocdest=254 if
  ...
  (Cencode80 & ocsrc==254) |
  ((Cencode00|Cencode10) & ocsrc==81)  | /* Real estate appraisers */
  ((Cencode00|Cencode10) & ocsrc==492);
...
replace ocdest=274 if
  ...
  (Cencode80 & ocsrc==284) |
  (Cencode80 & ocsrc==285) |
  ((Cencode00|Cencode10) & (ocsrc==474|ocsrc==475|ocsrc==476|ocsrc==484| ocsrc==485 | ocsrc==494 | ocsrc==496)) |
  (Cencode10 & ((ocsrc>=72.55 & ocsrc<=72.65) | ocsrc==496.5));
...
replace ocdest=276 if
  ...
  (Cencode70 & ocsrc==310) |
  (Cencode80 & ocsrc==276) |
  ((Cencode00|Cencode10) & ocsrc==472) |
  ((Cencode00|Cencode10) & ocsrc==513);  /* gaming cage workers (?) */
...
replace ocdest=303 if
  ...
  ((Cencode00|Cencode10) & ocsrc==500);    /* supvs of office/admin support work */
...
replace ocdest=313 if
  ...
  (Cencode70 & ocsrc==372)  |
  (Cencode80 & ocsrc==313)  |
  ((Cencode00|Cencode10) & (ocsrc==570| ocsrc==571|ocsrc==572|ocsrc==573|ocsrc==574)) ;
...
replace ocdest=315 if
  ...
  (Cencode70 & ocsrc==391) |
  (Cencode80 & ocsrc==315) |
  ((Cencode00|Cencode10) & (ocsrc==582 | ocsrc==583));  /* desktop publishers; should be in editors! */
                        /* but the 1990/2000 re-cats data don't go that way */
/* Interviewers, enumerators, and surveyors */
replace ocdest=316 if
  (Cencode70 & ocsrc==320) |
  (Cencode80 & ocsrc==316) |
  ((Cencode00|Cencode10) & ocsrc==523) | /* credit authorizers, checkers, & clerks */
  ((Cencode00|Cencode10) & ocsrc==531) |
  ((Cencode00|Cencode10) & ocsrc==534);  /* new accounts clerks */

/* Hotel clerks */
replace ocdest=317 if
  (Cencode80 & ocsrc==317) |
  ((Cencode00|Cencode10) & ocsrc==530);
...
replace ocdest=318 if
  ...
  (Cencode70 & ocsrc==390) |
  (Cencode80 & ocsrc==318) |
  ((Cencode00|Cencode10) & ocsrc==483) | /* travel agents */
  ((Cencode00|Cencode10) & ocsrc==541);  /* reservation, transport, ticket agents & clerks */
...
replace ocdest=319 if
  ...
  (Cencode70 & ocsrc==364) |
  (Cencode80 & ocsrc==319) |
  ((Cencode00|Cencode10) & ocsrc==540);   /* Receptionists and information clerks */
...
replace ocdest=326 if
  ...
  ((Cencode00|Cencode10) & (ocsrc==521 | ocsrc==535));
...
replace ocdest=329 if
  ...
  (Cencode70 & ocsrc==330) |
  (Cencode80 & ocsrc==329) |
  ((Cencode00|Cencode10) & ocsrc==244) |   /* library technicians */
  ((Cencode00|Cencode10) & ocsrc==532);
...
replace ocdest=335 if
  ...
  (Cencode70 & ocsrc==325) |
  (Cencode80 & ocsrc==335) |
  ((Cencode00|Cencode10) & ocsrc==526);

/* Records clerks */
replace ocdest=336 if
  (Cencode80 & ocsrc==336) |
  ((Cencode00|Cencode10) & ocsrc==520) |  /* brokerage clerks */
  ((Cencode00|Cencode10) & ocsrc==542);   /* info and rcord clerks, nec */
...
replace ocdest=346 if 
  (Cencode70 & ocsrc==332)   |
  (Cencode80 & ocsrc==346) |
  ((Cencode00|Cencode10) & ocsrc==556);
...
replace ocdest=347 if
  ...
  (Cencode70 & ocsrc==355) |
  (Cencode80 & ocsrc==347) |
  ((Cencode00|Cencode10) & ocsrc==590);    /* office machine operators */
...
replace ocdest=348 if
  ...
  (Cencode80 & ocsrc==348) |
  ((Cencode00|Cencode10) & ((ocsrc==501) | (ocsrc==502)));
...
replace ocdest=349 if
  ...
  ((Cencode00|Cencode10) & (ocsrc==503|ocsrc==504));
...
replace ocdest=359 if
  ...
  (Cencode70 & ocsrc==315) |
  (Cencode80 & ocsrc==359) |
  ((Cencode00|Cencode10) & (ocsrc==552|ocsrc==552.1|ocsrc==552.2));
...
replace ocdest=364 if
  ...
  ((Cencode00|Cencode10) & ((ocsrc==550) |   /* cargo and freight agents */
...
replace ocdest=365 if
  ...
  (Cencode80 & ocsrc==365) |
  ((Cencode00|Cencode10) & ocsrc==515) |  /* procurement clerks */
  ((Cencode00|Cencode10) & (ocsrc==562|ocsrc==964.5));   /* stock clerks and order fillers */
                              /* almost half of the 562s should go into 877 */
/* Meter readers */
replace ocdest=366 if
  (Cencode70 & ocsrc==334) |
  (Cencode80 & ocsrc==366) |
  ((Cencode00|Cencode10) & ocsrc==553);
...
replace ocdest=368 if
  ...
  ((Cencode00|Cencode10) & ocsrc==563); /* Weighers, measurers, checkers, samplers */
...
replace ocdest=373 if
  ...
  (Cencode80 & ocsrc==373) |
  ((Cencode00|Cencode10) & ((ocsrc==16) | (ocsrc==560)));
...
replace ocdest=375 if
  ...
  (Cencode70 & ocsrc==326) |
  (Cencode80 & ocsrc==375) |
  ((Cencode00|Cencode10) & ocsrc==54) | /* claims adjusters, appraisers, */
                               /*  examiners, investigators */
  ((Cencode00|Cencode10) & ocsrc==584);  /* insurance processing: policies and claims */
...
replace ocdest=376 if
  ...
  (Cencode80 & ocsrc==376) |
  ((Cencode00|Cencode10) & ocsrc==524) | /* customer service reps; not good fit */
                             /* but there isn't a simple service rep cat */
  ((Cencode00|Cencode10) & ocsrc==533); /* loan interviewers and clerks */

/* Eligibility clerks for govt programs; social welfare */
replace ocdest=377 if
  (Cencode80 & ocsrc==377) |
  ((Cencode00|Cencode10) & ocsrc==525);
...
replace ocdest=383 if
  ...
  (Cencode70 & ocsrc==301) |
  (Cencode80 & ocsrc==383) |
  ((Cencode00|Cencode10) & ocsrc==516);

/* Proofreaders */
replace ocdest=384 if
  (Cencode70 & ocsrc==362) |
  (Cencode80 & ocsrc==384) |
  ((Cencode00|Cencode10) & ocsrc==591);

/* Data entry keyers */
replace ocdest=385 if
  (Cencode70 & ocsrc==345) |   /* key punch operators */
  (Cencode80 & ocsrc==385) |
  ((Cencode00|Cencode10) & ocsrc==581);

/* Statistical clerks */
replace ocdest=386 if
  (Cencode70 & ocsrc==375) |
  (Cencode80 & ocsrc==386) |
  ((Cencode00|Cencode10) & ocsrc==592);

/* Teacher's aides */
replace ocdest=387 if
  (Cencode70 & ocsrc==382) |
  (Cencode80 & ocsrc==387) |
  (Cencode80>1 & ocsrc==467);   /* early childhood teacher's asst */
...
replace ocdest=389 if
  ...
  (Cencode80 & ocsrc==389) |
  ((Cencode00|Cencode10) & ocsrc==522) |  /* court, municipal, and license clerks */
  (Cencode00 & ocsrc==593) |
  (Cencode10 & (ocsrc==516.5|ocsrc==594));   /* office/admin support, nec */
...
replace ocdest=405 if
  ...
  (Cencode80 & ocsrc==449) |
  (Cencode80 & ocsrc==950) |
  ((Cencode00|Cencode10) & ocsrc==384) |  /* Flight attendants (?? yes, according to 1990/2000 map) */
  ((Cencode00|Cencode10) & ocsrc==423);
...
replace ocdest=415 if
  (Cencode80 & ocsrc==415) |
  ((Cencode00|Cencode10) & (ocsrc==373| ocsrc==372.5|ocsrc==373.5)) ;
...
replace ocdest=417 if
  ...
  (Cencode80 & ocsrc==416) |
  (Cencode80 & ocsrc==417) |
  ((Cencode00|Cencode10) & ocsrc==372) |
  ((Cencode00|Cencode10) & ocsrc==374) |
  ((Cencode00|Cencode10) & ocsrc==375);
...
replace ocdest=418 if
  ...
  (Cencode80 & ocsrc==414) |
  (Cencode80 & ocsrc==418) |
  ((Cencode00|Cencode10) & ocsrc==371) |
  ((Cencode00|Cencode10) & ocsrc==382) |
  ((Cencode00|Cencode10) & ocsrc==385) | /* Police officers */
  ((Cencode00|Cencode10) & ocsrc==386) |
  ((Cencode00|Cencode10) & ocsrc==387) |
  ((Cencode00|Cencode10) & ocsrc==391);  /* Private detectives and investigators */
...
replace ocdest=423 if
  ...
  ((Cencode00|Cencode10) & (ocsrc==370 | ocsrc==380 | ocsrc==380.1|ocsrc==380.2)) |
  ((Cencode00|Cencode10) & ocsrc==383);   /* fish and game wardens */

/* Crossing guards and bridge tenders */
replace ocdest=425 if
  (Cencode70 & ocsrc==960) |
  (Cencode80 & ocsrc==425) |
  ((Cencode00|Cencode10) & ocsrc==394);
...
replace ocdest=426 if
  ...
  (Cencode80 & ocsrc==426) |
  (Cencode00 & ocsrc==392) |
  (Cencode10 & (ocsrc==393|ocsrc==394.5)); /*transpportation security screeners*/
...
replace ocdest=427 if
  (Cencode80 & ocsrc==427) |
  /*(Cencode00 & ocsrc==373) |*/
  ((Cencode00|Cencode10) & ocsrc==390) |
  (Cencode00 & ocsrc==395) |
  (Cencode10 & (ocsrc==395.5| ocsrc==394.6 | ocsrc==396)) ;
...
replace ocdest=436 if
  ...
  (Cencode80 & ocsrc==404) |
  ((Cencode00|Cencode10) & ((ocsrc==400) | (ocsrc==401) | (ocsrc==402)));
...
replace ocdest=439 if
  ...
  (Cencode80 & ocsrc==439) |
  ((Cencode00|Cencode10) & ocsrc==405);    /* combined food prep and serving workers */

/* Waiter/waitress's assistant */
replace ocdest=443 if
  (Cencode70 & ocsrc==911) |   /* busboys */
  (Cencode80 & ocsrc==443) |
  ((Cencode00|Cencode10) & (ocsrc==406| ocsrc==405.5)) |   /* cafeteria counter attendants */
  ((Cencode00|Cencode10) & ocsrc==412) |   /* food servers outside restaurants */
  ((Cencode00|Cencode10) & ocsrc==413);    /* dining room / cafe / bar helpers */

/* Misc food prep workers */
replace ocdest=444 if
  (Cencode80 & ocsrc==444) |
  ((Cencode00|Cencode10) & ocsrc==403) | /* food preparation workers */
  ((Cencode00|Cencode10) & ocsrc==414) | /* dishwashers */
  ((Cencode00|Cencode10) & ocsrc==416);  /* all other food prep and serving workers */

/* Dental assistants */
replace ocdest=445 if
  (Cencode70 & ocsrc==921) |
  (Cencode80 & ocsrc==445) |
  ((Cencode00|Cencode10) & ocsrc==364);
...
replace ocdest=446 if
  ...
  (Cencode80 & ocsrc==446) |
  (Cencode00 & ocsrc==365) |
  (Cencode10 & (ocsrc>=364.5 & ocsrc<=365.5)); /* med. assistants are broken down further here*/
...
replace ocdest=447 if
  ...
  (Cencode70 & ocsrc==925) |
  (Cencode80 & ocsrc==447) |
  ((Cencode00|Cencode10) & (ocsrc==360 | ocsrc==360.1|ocsrc==360.3|ocsrc==360.5)) | /* Nursing,psychiatric,& home health aides */
  ((Cencode00|Cencode10) & (ocsrc==461|ocsrc==360.2));

/* Supervisors of cleaning and building service workers */
replace ocdest=448 if
  (Cencode80 & ocsrc==448) |
  ((Cencode00|Cencode10) & ocsrc==420);
...
replace ocdest=454 if
  ...
  (Cencode70 & ocsrc==943) |
  (Cencode80 & ocsrc==454) |
  ((Cencode00|Cencode10) & (ocsrc==975|ocsrc==976));     /* misc material moving */

/* Pest control occupations */
replace ocdest=455 if 
  (Cencode80 & ocsrc==455) | 
  ((Cencode00|Cencode10) & ocsrc==424);

/* Supervisors of personal service jobs */
replace ocdest=456 if
  (Cencode80 & ocsrc==456) |
  ((Cencode00|Cencode10) & (ocsrc==432|ocsrc==433));
...
replace ocdest=458 if
  ...
  (Cencode70 & ocsrc==944) |
  (Cencode80 & ocsrc==458) |
  ((Cencode00|Cencode10) & ocsrc==451) |
  ((Cencode00|Cencode10) & (ocsrc==452|ocsrc==452.1|ocsrc==452.2|ocsrc==452.5));
...
replace ocdest=459 if
  ...
  (Cencode70 & ocsrc==932) |
  (Cencode80 & ocsrc==459) |
  ((Cencode00|Cencode10) & ocsrc==440) |
  ((Cencode00|Cencode10) & (ocsrc==443|ocsrc==443.5));

/* Guides */
replace ocdest=461 if
  (Cencode80==1 & ocsrc==463) |
  (Cencode80>1 & ocsrc==461) |  /* remapped between 1980 and 1990 */
  ((Cencode00|Cencode10) & ocsrc==454) ;

/* Ushers */   /* remapped in 1990 Census */
replace ocdest=462 if
  (Cencode60 & ocsrc==874) |
  (Cencode70 & ocsrc==953) |
  (Cencode80==1 & ocsrc==464) |
  (Cencode80>=2 & ocsrc==462) |
  ((Cencode00|Cencode10) & ocsrc==442);
...
replace ocdest=463 if
  ...
  (Cencode80==1 & ocsrc==465) |
  (Cencode80>1 & ocsrc==463)  |
  (Cencode00 & ocsrc==455) |   /* Transportation attendants */
  ((Cencode00|Cencode10) & ocsrc==941) |
  (Cencode10 & (ocsrc==905|ocsrc==941.5));   /* Transportation inspectors */
...
replace ocdest=464 if
  ...
  (Cencode70 & ocsrc==934) |
  (Cencode80==1 & ocsrc==466) |
  (Cencode80>1 & ocsrc==464) |
  ((Cencode00|Cencode10) & ocsrc==453);

/* Welfare service aides */   /* remapped in 1990 Census */
replace ocdest=465 if
  (Cencode70 & ocsrc==954) |
  (Cencode80==1 & ocsrc==467) |
  (Cencode80>=2 & ocsrc==465) |
  (Cencode00 & ocsrc==202) |
  (Cencode10 & (ocsrc>=201.5&ocsrc<=202.5));
...
replace ocdest=468 if
  ...
  (Cencode80>1 & ocsrc==466) |
  ((Cencode00|Cencode10) & ocsrc==460) |
  ((Cencode00|Cencode10) & ocsrc==464);    /* residential advisors */
...
replace ocdest=469 if
  ...
  (Cencode70 & ocsrc==986) |
  (Cencode80 & ocsrc==469) |
  ((Cencode00|Cencode10) & ocsrc==363) |  /* massage therapists */
  ((Cencode00|Cencode10) & ocsrc==415) |  /* host/hostesses in restaurants/lounge/coffee shops */
  ((Cencode00|Cencode10) & (ocsrc==446|ocsrc==446.1)) |  /* funeral service workers */
  ((Cencode00|Cencode10) & (ocsrc==465|ocsrc==465.5));
...
replace ocdest=473 if
  ...
  (Cencode80 & ocsrc==473) |
  (Cencode00 & ocsrc==21) |
  (Cencode10 & ocsrc==20.5); /*farmersand ranchers get grouped with their managers in 2012, we put them all here*/
...
replace ocdest=475 if
  ...
  (Cencode80 & ocsrc==475) |
  (Cencode00 & ocsrc==20)  |
  ((Cencode00|Cencode10) & ocsrc==602);   /* animal breeders */
...
replace ocdest=479 if
  ...
  (Cencode80 & ocsrc==479) |
  ((Cencode00|Cencode10) & ocsrc==434) | /* animal trainers */
  ((Cencode00|Cencode10) & ocsrc==605);  /* misc agricultural workers */
...
replace ocdest=485 if
  (Cencode80 & ocsrc==485) |
  ((Cencode00|Cencode10) & ocsrc==421);  /* supvs of landscaping, lawn svc, groundskeeping */
...
replace ocdest=486 if
  ...
  (Cencode70 & ocsrc==755) |
  (Cencode80 & ocsrc==486) |
  ((Cencode00|Cencode10) & (ocsrc==425|ocsrc==425.1|ocsrc==425.2|ocsrc==425.5));

/* Animal caretakers, except farm */
replace ocdest=487 if
  (Cencode70 & ocsrc==740) |
  (Cencode80 & ocsrc==487) |
  ((Cencode00|Cencode10) & ocsrc==435);
...
replace ocdest=488 if
  ...
  (Cencode80 & ocsrc==488) |
  ((Cencode00|Cencode10) & ocsrc==604);

/* Inspectors of agricultural products */
replace ocdest=489 if
  (Cencode80 & ocsrc==489) |
  ((Cencode00|Cencode10) & ocsrc==601);
...
replace ocdest=496 if
  ...
  (Cencode80 & ocsrc==496) |
  (Cencode00 & ocsrc==600) |
  ((Cencode00|Cencode10) & ocsrc==612) |
  ((Cencode00|Cencode10) & ocsrc==613) |
  (Cencode10 & ocsrc==600.5);
...
replace ocdest=498 if
  ...
  (Cencode80 & ocsrc==498) | 
  (Cencode80 & ocsrc==499) |
  ((Cencode00|Cencode10) & ocsrc==610) | 
  ((Cencode00|Cencode10) & (ocsrc==611|ocsrc==611.5));

/* Supervisors of mechanics and repairers */
replace ocdest=503 if
  (Cencode80 & ocsrc==503) |
  ((Cencode00|Cencode10) & ocsrc==700);
...
replace ocdest=505 if
  ...
  (Cencode80 & ocsrc==505) |
  (Cencode80 & ocsrc==506) |
  ((Cencode00|Cencode10) & ocsrc==720);

/* Bus, truck, and stationary engine mechanics */
replace ocdest=507 if 
  (Cencode80 & ocsrc==507) |
  ((Cencode00|Cencode10) & ocsrc==721);
...
replace ocdest=516 if
  ...
  (Cencode80 & ocsrc==516) | 
  (Cencode80 & ocsrc==517) |
  ((Cencode00|Cencode10) & ocsrc==722) | 
  ((Cencode00|Cencode10) & ocsrc==726);

/* Industrial machinery repair */
replace ocdest=518 if
  (Cencode80 & ocsrc==518) |
  ((Cencode00|Cencode10) & ocsrc==733);
...
replace ocdest=523 if
  ...
  (Cencode70 & ocsrc==485) |
  (Cencode80 & ocsrc==523) |
  ((Cencode00|Cencode10) & ocsrc==710) |
  ((Cencode00|Cencode10) & ocsrc==712);

/* Data processing equipment repairers */
replace ocdest=525 if
  (Cencode70 & ocsrc==475) |
  (Cencode80 & ocsrc==525) |
  ((Cencode00|Cencode10) & ocsrc==701);  /* computer, automatic teller, office machine repairers */
...
replace ocdest=527 if
  ...
  ((Cencode00|Cencode10) & ((ocsrc==702) | (ocsrc==742)));
...
replace ocdest=533 if
  ...
  (Cencode80 & ocsrc==703) |
  (Cencode80 & ocsrc==704) |
  ((Cencode00|Cencode10) & ocsrc==703) |
  ((Cencode00|Cencode10) & ocsrc==705) |  /* installers and repairers of electronics on transportation equipment */
  ((Cencode00|Cencode10) & ocsrc==711);
...
replace ocdest=535 if
  ...
  (Cencode80 & ocsrc==535) |
  (Cencode80 & ocsrc==647) |
  ((Cencode00|Cencode10) & ocsrc==743) |  /* precision instrument and equipment repairers */
  ((Cencode00|Cencode10) & ocsrc==875);

/* Locksmiths and safe repairers */
replace ocdest=536 if
  (Cencode80 & ocsrc==536) |
  ((Cencode00|Cencode10) & ocsrc==754);
...
replace ocdest=539 if
  (Cencode80 & ocsrc==539) |
  ((Cencode00|Cencode10) & ocsrc==730);

/* Elevator installers and repairers */
replace ocdest=543 if
  (Cencode80 & ocsrc==543) |
  ((Cencode00|Cencode10) & ocsrc==670);
...
replace ocdest=549 if
  ...
  ((Cencode00|Cencode10) & ocsrc==734) |
  ((Cencode00|Cencode10) & ((ocsrc==755) | (ocsrc==756))) |
  (Cencode00 & ocsrc==762) |
  (Cencode10 & (ocsrc==744|ocsrc==763|ocsrc==654| ocsrc==764)); /*we include the new 2010 occ "solar photovoltaic installers", which comes in part from 5 different occupations and is difficult to map cleanly*/
...
replace ocdest=563 if
  ...
  ((Cencode00|Cencode10) & ocsrc==622) | 
  ((Cencode00|Cencode10) & ocsrc==624) ;
...
replace ocdest=575 if
  ...
  ((Cencode00|Cencode10) & ((ocsrc==713) | (ocsrc==635))) |
  (Cencode10 & (ocsrc==651.5|ocsrc==635.5)) ;

/* Electrical power installers and repairers */
replace ocdest=577 if
  (Cencode70 & ocsrc==433) |
  (Cencode80 & ocsrc==577) |
  ((Cencode00|Cencode10) & ocsrc==704) |  /* electric motor, power tool, etc, repairers */
  ((Cencode00|Cencode10) & ocsrc==741) |
  ((Cencode00|Cencode10) & ocsrc==760);   /* signal and track switch repairers */
...
replace ocdest=579 if
  ...
  (Cencode80 & ocsrc==579) |
  ((Cencode00|Cencode10) & (ocsrc==642|ocsrc==641));
...
replace ocdest=585 if
  ...
  ((Cencode00|Cencode10) & (ocsrc==644|ocsrc==644.1|ocsrc==644.2));
...
replace ocdest=593 if
  ...
  (Cencode70 & ocsrc==601) |
  (Cencode80 & ocsrc==593) |
  ((Cencode00|Cencode10) & ocsrc==640) |
  ((Cencode00|Cencode10) & ocsrc==672);     /* hazardous material remover workers */
...
replace ocdest=594 if
  ...
  (Cencode80 & ocsrc==594) |
  (Cencode80 & ocsrc==855) |
  ((Cencode00|Cencode10) & (ocsrc==630|ocsrc==630.5));
...
replace ocdest=595 if
  ...
  (Cencode80 & ocsrc==595) |
  (Cencode00 & ocsrc==651) |
  (Cencode10 & ocsrc==651.5) ;

/* Sheet metal duct installers */
replace ocdest=596 if
  (Cencode80 & ocsrc==596) |
  ((Cencode00|Cencode10) & ocsrc==652);
...
replace ocdest=597 if
  ...
  (Cencode70 & ocsrc==550) |
  (Cencode80 & ocsrc==597) |
  ((Cencode00|Cencode10) & ocsrc==650) |  /* reinforcing iron and rebar workers */
  ((Cencode00|Cencode10) & ocsrc==653) |  /* structural iron and steel wkrs */
  ((Cencode00|Cencode10) & ocsrc==774);   /* structural metal fabricators and fitters */

/* Drillers of earth */
replace ocdest=598 if
  (Cencode70 & ocsrc==614) |
  (Cencode80 & ocsrc==598) |
  ((Cencode00|Cencode10) & (ocsrc==682|ocsrc==682.5));
...
replace ocdest=599 if
  ...
  (Cencode80 & ocsrc==599) |
  ((Cencode00|Cencode10) & ocsrc==631) |  /* pile-driver operators */
  ((Cencode00|Cencode10) & ocsrc==671) |  /* fence erectors */
  (Cencode00 & ocsrc==676) |
  (Cencode10 & ocsrc==676.5) ;

/* Drillers of oil wells */
replace ocdest=614 if
  (Cencode80 & ocsrc==614) |
  ((Cencode00|Cencode10) & ocsrc==680) |
  ((Cencode00|Cencode10) & ocsrc==692);  /* roustabouts, oil and gas */
...
replace ocdest=615 if
  ...
  (Cencode80 & ocsrc==615) |
  ((Cencode00|Cencode10) & (ocsrc==683|ocsrc==683.5));
...
replace ocdest=616 if
  ...
  (Cencode70 & ocsrc==640) |
  (Cencode80 & ocsrc==616) |
  ((Cencode00|Cencode10) & (ocsrc==684|ocsrc==685));

/* Other mining occupations */
replace ocdest=617 if
  (Cencode80 & ocsrc==617) |
  ((Cencode00|Cencode10) & ocsrc==691) |  /* roof bolters, mining */
  ((Cencode00|Cencode10) & (ocsrc==694|ocsrc==695));
...
replace ocdest=628 if
  ...
  (Cencode80>1 & ocsrc==628) |  /* 1990 supervisor of production occs */
  (Cencode80==1 & ocsrc==633) |  /* 1980 supervisor of production occs */
  (Cencode80==1 & ocsrc==863) |
  ((Cencode00|Cencode10) & ocsrc==770);
...
replace ocdest=645 if
  ...
  (Cencode80 & ocsrc==656) |
  (Cencode80 & ocsrc==676) |
  ((Cencode00|Cencode10) & ocsrc==806) |
  ((Cencode00|Cencode10) & ocsrc==844) |  /* fabric and apparel patternmakers */
  ((Cencode00|Cencode10) & ocsrc==852);   /* wooden model makers and patternmakers */

/* Lay-out workers */
replace ocdest=646 if
  (Cencode70 & ocsrc==540) |  /* shipfitters */
  (Cencode80 & ocsrc==646) |
  ((Cencode00|Cencode10) & ocsrc==816);
...
replace ocdest=649 if
  ...
  (Cencode70 & ocsrc==435) |
  (Cencode80 & ocsrc==649) |
  ((Cencode00|Cencode10) & ocsrc==891); /* Etchers and engravers */
...
replace ocdest=657 if
  ...
  ((Cencode00|Cencode10) & ocsrc==850);

/* Furniture and wood finishers */
replace ocdest=658 if
  (Cencode70 & ocsrc==443) |
  (Cencode80 & ocsrc==658) |
  ((Cencode00|Cencode10) & ocsrc==851);
...
replace ocdest=668 if
  ...
  (Cencode70 & ocsrc==563) |
  (Cencode80 & ocsrc==668) |
  ((Cencode00|Cencode10) & ocsrc==845);

/* Shoe repairers */
replace ocdest=669 if
  (Cencode70 & ocsrc==542) |
  (Cencode80 & ocsrc==669) |
  ((Cencode00|Cencode10) & (ocsrc==833|ocsrc==833.5));
...
replace ocdest=678 if
  ...
  (Cencode80 & ocsrc==678) |
  (Cencode00 & ocsrc==341) |
  ((Cencode00|Cencode10) & ocsrc==876) |
  (Cencode10 & (ocsrc==342| (ocsrc>=342.1 & ocsrc<=342.4) | ocsrc==343)) ;
...
replace ocdest=679 if
  ...
  (Cencode80 & ocsrc==679) |
  (Cencode00 & ocsrc==823) |
  (Cencode10 & (ocsrc>=825.55 & ocsrc<=825.65)) ;

/* Other precision and craft workers */
replace ocdest=684 if
  (Cencode60 & ocsrc==545) |  /* craftspersons, nec */
  (Cencode80 & ocsrc==684) |
  ((Cencode00|Cencode10) & ocsrc==812);   /* multiple machine tool setters, operators, and tenders, metal and plastic */
...
replace ocdest=687 if
  ...
  (Cencode70 & ocsrc==402) |
  (Cencode80 & ocsrc==687) |
  ((Cencode00|Cencode10) & ocsrc==780);

/* Batch food makers */
replace ocdest=688 if
  (Cencode80 & ocsrc==688) |
  ((Cencode00|Cencode10) & ocsrc==784);
...
replace ocdest=703 if
  ...
  ((Cencode00|Cencode10) & ((ocsrc==703) | (ocsrc==704))) |
        /* Lathe and turning machine set-up and operators */
  ((Cencode00|Cencode10) & ocsrc==801) |
  ((Cencode00|Cencode10) & (ocsrc==802|ocsrc==802.5));   /* milling and planing machine setters, operators, and tenders, metal and other */
...
replace ocdest=713 if
  ...
  (Cencode70 & ocsrc==442) |
  (Cencode80 & ocsrc==713) |
  ((Cencode00|Cencode10) & (ocsrc==793|ocsrc==792.5));
...
replace ocdest=724 if
  ...
  (Cencode80 & ocsrc==724) |
  ((Cencode00|Cencode10) & ocsrc==815);

/* Wood lathe, routing, and planing machine operators */
replace ocdest=726 if 
  (Cencode80 & ocsrc==726) |
  ((Cencode00|Cencode10) & (ocsrc==822|ocsrc==822.5));
...
replace ocdest=729 if
  (Cencode80 & ocsrc==729) |
  ((Cencode00|Cencode10) & ocsrc==854);

/* Other woodworking machine operators */
replace ocdest=733 if
  (Cencode80 & ocsrc==733) |
  ((Cencode00|Cencode10) & (ocsrc==855|ocsrc==855.5));
...
replace ocdest=748 if
  ...
  ((Cencode00|Cencode10) & ocsrc==830);  /* Launderers and ironers */
...
replace ocdest=749 if
  ...
  (Cencode80 & ocsrc==749) |
  ((Cencode00|Cencode10) & (ocsrc==836| ocsrc==836.5)) | /* textile bleaching & dyeing machine operators */
  ((Cencode00|Cencode10) & (ocsrc==846| ocsrc==846.5));

/* Cementing and gluing machine operators */
replace ocdest=753 if
  (Cencode80 & ocsrc==753) |
  ((Cencode00|Cencode10) & ocsrc==885);
...
replace ocdest=754 if
  ...
  (Cencode70 & ocsrc==643) |
  (Cencode80 & ocsrc==754) |
  ((Cencode00|Cencode10) & ocsrc==880);

/* Extruding, forming, and compressing machine operators */
replace ocdest=755 if
  (Cencode80 & ocsrc==755) |
  (Cencode80 & ocsrc==758) |
  ((Cencode00|Cencode10) & ocsrc==792) |
  ((Cencode00|Cencode10) & ocsrc==843) |
  ((Cencode00|Cencode10) & ocsrc==872);
...
replace ocdest=756 if
  ...
  (Cencode70 & ocsrc==641) |
  (Cencode80 & ocsrc==756) |
  ((Cencode00|Cencode10) & ocsrc==865);    /* crushing, grinding, polishing, mixing, and blending workers */

/* Separating, filtering, and clarifying machine operators */
replace ocdest=757 if
  (Cencode80 & ocsrc==757) |
  ((Cencode00|Cencode10) & ocsrc==864);
...
replace ocdest=766 if
  ...
  (Cencode80 & ocsrc==766) |
  ((Cencode00|Cencode10) & ocsrc==804) |
  ((Cencode00|Cencode10) & ocsrc==873);
...
replace ocdest=769 if
  ...
  (Cencode80 & ocsrc==769) |
  ((Cencode00|Cencode10) & ocsrc==785) |
  ((Cencode00|Cencode10) & ocsrc==871);
...
replace ocdest=779 if
  ...
  ((Cencode00|Cencode10) & (ocsrc==884|ocsrc==899)) | /* semiconductor processing jobs */
  ((Cencode00|Cencode10) & ocsrc==890) | /* cooling and freezing equipment operators and tenders */
  ((Cencode00|Cencode10) & ocsrc==894) | /* tire builders */
  (Cencode00 & ocsrc==896) |
  (Cencode10 & (ocsrc==785.5|ocsrc==896.5|ocsrc==790.5));  /* other production workers incl semic */
...
replace ocdest=785 if
  ...
  (Cencode80 & ocsrc==683) |
  (Cencode80 & ocsrc==785) |
  ((Cencode00|Cencode10) & ((ocsrc==771) | (ocsrc==772)|(ocsrc==773) | (ocsrc==775)));
...
replace ocdest=799 if
  ...
  ((Cencode00|Cencode10) & ocsrc==874);

/* Supervisors of transportation; of motor vehicle operators */
replace ocdest=803 if
  (Cencode80 & ocsrc==803) |
  (Cencode80>1 & ocsrc==864) |  /* supvs of equipment handling and cleaning */
  ((Cencode00|Cencode10) & (ocsrc==900|ocsrc==900.5));
...
replace ocdest=804 if
  ...
  ((Cencode00|Cencode10) & ((ocsrc==751) | (ocsrc==913) | (ocsrc==960)));
...
replace ocdest=808 if
  ...
  (Cencode70 & ocsrc==703) |
  (Cencode80 & ocsrc==808) |
  ((Cencode00|Cencode10) & (ocsrc==912|ocsrc==912.1|ocsrc==912.2));
...
replace ocdest=809 if
  ...
  (Cencode80 & ocsrc==809) |
  (Cencode80 & ocsrc==814) |
  ((Cencode00|Cencode10) & ocsrc==911) |  /* ambulance drivers, not counting emergency medical techs */
  ((Cencode00|Cencode10) & (ocsrc==914|ocsrc==914.1|ocsrc==914.2)) |
  ((Cencode00|Cencode10) & ocsrc==915);

/* Parking lot attendants */
replace ocdest=813 if
  (Cencode70 & ocsrc==711) |
  (Cencode80 & ocsrc==813) |
  ((Cencode00|Cencode10) & ocsrc==935);
...
replace ocdest=824 if
  ...
  (Cencode80 & ocsrc==824) |
  (Cencode80 & ocsrc==826) |
  ((Cencode00|Cencode10) & (ocsrc==920|ocsrc==921)) | /* locomotive engineers and operators */
  ((Cencode00|Cencode10) & (ocsrc==926|ocsrc==926.5)); /* subway, streetcar, and other rail transport jobs */
...
replace ocdest=829 if
  ...
  (Cencode80 & ocsrc==833) |
  ((Cencode00|Cencode10) & ((ocsrc==930) | (ocsrc==931) | (ocsrc==933)));
...
replace ocdest=834 if
  ...
  ((Cencode00|Cencode10) & ocsrc==934);   /* bridge and lock tenders */
...
replace ocdest=848 if
  ...
  ((Cencode00|Cencode10) & ocsrc==951) | /* crane and tower operators */
  ((Cencode00|Cencode10) & (ocsrc==956|ocsrc==957));  /* hoist and winch operators */

/* Excavating and loading machine operators */
replace ocdest=853 if
  (Cencode60 & ocsrc==425) |
  (Cencode80 & ocsrc==853) | /* excavating and loading machine operators */
  ((Cencode00|Cencode10) & (ocsrc==952|ocsrc==682.1)); /* dredge, excavating, and loading machine operators */
...
replace ocdest=859 if
  ...
  ((Cencode00|Cencode10) & ocsrc==965) | /* pumping station operators */
  ((Cencode00|Cencode10) & ocsrc==973);  /* shuttle car operators */

/* Helpers, construction */
replace ocdest=865 if
  (Cencode80 & ocsrc==865) |
  ((Cencode00|Cencode10) & ocsrc==761);

/* Helpers, surveyors */
replace ocdest=866 if
  (Cencode70 & ocsrc==605) | /* chainment, rodmen, and axemen, surveying */
  (Cencode80 & ocsrc==866) |
  ((Cencode00|Cencode10) & ocsrc==660);
...
replace ocdest=869 if
  ...
  (Cencode70 & ocsrc==751) |
  (Cencode80 & ocsrc==869) |
  ((Cencode00|Cencode10) & ocsrc==626) |
  ((Cencode00|Cencode10) & ocsrc==673) |  /* highway maintenance workers */
  ((Cencode00|Cencode10) & ocsrc==693);   /* helpers to extraction workers */

/* Production helpers */   /* changed from 873 to 874 on 12/15/2008 */
replace ocdest=874 if
  (Cencode80 & ocsrc==873) |
  (Cencode80 & ocsrc==874) |
  ((Cencode00|Cencode10) & ocsrc==895);

/* Garbage collectors */
replace ocdest=875 if
  (Cencode70 & ocsrc==754) |
  (Cencode80 & ocsrc==875) |
  ((Cencode00|Cencode10) & ocsrc==972);
...
replace ocdest=876 if
  ...
  ((Cencode00|Cencode10) & (ocsrc==950|ocsrc==974));

/* Stock handlers [and baggers, post-1982] */
replace ocdest=877 if
  (Cencode70 & ocsrc==762) |
  (Cencode80 & ocsrc==877) |
  ((Cencode00|Cencode10) & ocsrc==962);

/* Machine feeders and offbearers */
replace ocdest=878 if
  (Cencode80 & ocsrc==878) |
  ((Cencode00|Cencode10) & ocsrc==963);
...
replace ocdest=883 if
  ...
  ((Cencode00|Cencode10) & (ocsrc==942|ocsrc==936.5|ocsrc==943));  /* misc transport workers, incl bridge/lock tenders */
...
replace ocdest=885 if
  ...
  (Cencode70 & ocsrc==623) |
  (Cencode80 & ocsrc==885) |
  ((Cencode00|Cencode10) & (ocsrc==936|ocsrc==936.5));
...
replace ocdest=889 if
  ...
  (Cencode80>1 & ocsrc==868) |
  (Cencode80>1 & ocsrc==874) |
  (Cencode80 & ocsrc==889) |
  ((Cencode00|Cencode10) & ocsrc==674) |
  ((Cencode00|Cencode10) & ocsrc==675) |
  ((Cencode00|Cencode10) & ocsrc==962);
...
replace ocdest=905 if
  ...
  ((Cencode00|Cencode10) & (ocsrc==980|ocsrc==981|ocsrc==982|ocsrc==982.5|ocsrc==983|ocsrc==984));


/* Unemployed */
replace ocdest=991 if
  (Cencode70 & ocsrc==991) |
  ((Cencode00|Cencode10) & ocsrc==992);
...
assert ocdest ~= 0;
assert ocdest ~= .;
gen tst1 = occ1950 == 595;
gen tst2 = ocdest == 905;
capture assert tst1 == tst2;

* !! there are three cases in 1970 CPS of people with job 555 (former armed forces)
* !! which are mapped to 'unknown' in occ1950.  I could do that but it seems
* !! like this is reasonable too.  I'm remaining open.
...
replace mjrocc=1 if ocdest>=3 & ocdest<=37 /*executive, administrative, and managerial occupations */
replace mjrocc=2 if ocdest>=43 & ocdest<=199 /* professional specialty occupations */
replace mjrocc=3 if ocdest>=203 & ocdest<=235 /* technicians and related support occupations */
replace mjrocc=4 if ocdest>=243 & ocdest<=285 /* sales occupations*/
replace mjrocc=5 if ocdest>=303 & ocdest<=389 /* administrative support occupations, including clerical*/
replace mjrocc=6 if ocdest>=403 & ocdest<=407 /* private household occupations */
replace mjrocc=7 if ocdest>=413 & ocdest<=427 /* protective service occupations */
replace mjrocc=8 if ocdest>=433 & ocdest<=469 /* service occupations, except protective & hhsld */
replace mjrocc=9 if ocdest>=503 & ocdest<=699 /* precision production, craft, and repair occupations */
replace mjrocc=10 if ocdest>=703 & ocdest<=799 /* machine operators, assemblers, & inspectors */
replace mjrocc=11 if ocdest>=803 & ocdest<=859 /* transportation and material moving occupations */
replace mjrocc=12 if ocdest>=864 & ocdest<=889 /* handlers, equipmnt cleaners, helpers, laborers */
replace mjrocc=13 if ocdest>=473 & ocdest<=499 /* farming, forestry, and fishing occupations */
replace mjrocc=14 if ocdest == 905 /* armed forces */

...
twoway ///
    (scatter mean_unemployed jtyears, mcolor(gs8) msymbol(o)) ///
    (lowess mean_unemployed jtyears, lcolor(blue) lwidth(medium)) ///
...
use fullcps_jtyears, clear

gen bin = "0-1" if jtyears==1
replace bin = "1-2" if jtyears==2
replace bin = "2-3" if jtyears==3
replace bin = "3-5" if inrange(jtyears, 4, 5)
replace bin = "5-8" if inrange(jtyears, 5, 8)
replace bin = "8-10" if inrange(jtyears, 9, 10)
replace bin = "10-15" if inrange(jtyears, 11, 15)
replace bin = "15-20" if inrange(jtyears, 16, 20)
replace bin = "21-25" if inrange(jtyears, 21, 25)
replace bin = ">25" if jtyears>25

collapse(mean) mean_unemployed= f12unemployed (semean) se_unemployed= f12unemployed [aw=jtsuppwt], by(bin)
